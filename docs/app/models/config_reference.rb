...
module ConfigReference
  GROUPS = {
    ...
    "Streams (SSE)" => [
      ...
      { name: "streams_pool_autoscale", type: "Boolean", default: "false", desc: "Self-tuning: grow the streams pool into a fair share of live Postgres connection headroom under saturation, shrink back to streams_pool_size when idle, and emergency-shrink if the DB runs low on connections. Opt-in; no-op on the shared-ActiveRecord path." },
