...
3. Resolve every conflicted file **semantically** — read both sides and produce the version that preserves BOTH changes' intent. Never blanket `--ours`/`--theirs` a source file. Repo-specific rules:
   ...
   - **`lib/pgbus/version.rb`**: releases land DIRECTLY on `main` via `rake release[X.Y.Z]` (no PR), and `RELEASING.md` says even the release-prep PR leaves `version.rb` alone — so an ordinary branch never edits this file. A conflict here means the branch edited it against convention: take the BASE's version unless the branch's own commits show a deliberate, explained bump; if the intent isn't obvious, stop and ask.
   - **The frozen lockfiles** — `docs/Gemfile.lock` and `gemfiles/rails_7_1.gemfile.lock` (both tracked; the gem root's `Gemfile.lock` is gitignored and can never conflict): NEVER hand-merge conflict markers, and NEVER run `bundle lock` — the docs lock's broad PLATFORMS list makes a full re-resolve fail on platform-only gems (see the Rakefile's release task and `spec/pgbus/frozen_lockfile_sync_spec.rb`). If the conflict is only the `pgbus (X.Y.Z)` path-gem pin, take the base's file and edit the pin line(s) by hand to match `lib/pgbus/version.rb` (the spec's failure message shows exactly this fix). If the branch changed real dependencies, take the base's file and re-resolve on top: `cd docs && bundle install` for the docs lock, `BUNDLE_GEMFILE=gemfiles/rails_7_1.gemfile bundle install` for the Rails 7.1 lock.
...
4. Run the verification gates BEFORE pushing the merge — scoped to what the conflict touched, at minimum:
   ...
   bundle exec rspec spec/pgbus/ spec/generators/
