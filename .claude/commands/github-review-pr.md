...
3. Resolve every conflicted file **semantically** — read both sides and produce the version that preserves BOTH changes' intent. Never blanket `--ours`/`--theirs` a source file. Repo-specific rules:
   ...
   - **The frozen lockfiles** — `Gemfile.lock`, `gemfiles/rails_7_1.gemfile.lock` and `docs/Gemfile.lock` (all three tracked and installed frozen in CI): NEVER hand-merge conflict markers, and NEVER run `bundle lock` — the docs lock's broad PLATFORMS list makes a full re-resolve fail on platform-only gems (see the Rakefile's release task and `spec/pgbus/frozen_lockfile_sync_spec.rb`). If the conflict is only the `pgbus (X.Y.Z)` path-gem pin, take the base's file and edit the pin line(s) by hand to match `lib/pgbus/version.rb` (the spec's failure message shows exactly this fix). If the branch changed real dependencies, take the base's file and re-resolve on top: `bundle install` for the root lock, `cd docs && bundle install` for the docs lock, `BUNDLE_GEMFILE=gemfiles/rails_7_1.gemfile bundle install` for the Rails 7.1 lock.
