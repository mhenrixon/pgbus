...
# the frozen install instant-fails (the root lock on every main-Gemfile leg and
# in release.yml; the Rails 7.1 leg with exit 16; docs-CI on any docs change).
# `rake release` regenerates them in the bump commit, but this spec catches the
# drift directly — and in the normal rspec run, not just when a frozen CI leg
# happens to fail. See #338 (the release-task sync) and its fix.
#
...
RSpec.describe "Frozen lockfile version sync" do
  %w[Gemfile.lock gemfiles/rails_7_1.gemfile.lock docs/Gemfile.lock].each do |relative|
