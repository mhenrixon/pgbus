...
# v1.11.0 in the tracking table, seed a queue with messages, then run the exact
# SQL the generated `upgrade_pgmq` migration emits to move to v1.11.1
# (drop functions -> re-create at target version -> record the upgrade).
#
# After the upgrade the tracking table must report v1.11.1, existing queues and
# messages must survive, the new read_grouped_head function must be available,
# and the core pgmq functions must remain callable.
#
...
RSpec.describe "PGMQ schema upgrade path (integration)", :integration do
  ...
  let(:from_version) { "1.11.0" }
  let(:to_version)   { "1.11.1" }
  ...
  before do
    # Establish a clean v1.11.0 baseline (functions + tracking record) and seed
  ...
  it "records v1.11.0 before the upgrade" do
  ...
  it "advances the tracking table to v1.11.1 after the upgrade" do
  ...
  it "makes the new read_grouped_head function available after the upgrade" do
    run_upgrade_migration_sql(to_version)

    expect(function_exists?("read_grouped_head")).to be(true)
  end
